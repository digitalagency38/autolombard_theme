<?php
require_once( $_SERVER['DOCUMENT_ROOT'] . '/wp-load.php' ); /* добавить полный путь*/
global $wp;
global $wpdb;
...
$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}car_mark WHERE id_car_mark = " . $brand . "", OBJECT );	
$text .= "\n" . "Марка: " . "*" . $results[0]->name; 	
}
if ($model){
$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}car_model WHERE id_car_model = " . $model . "", OBJECT );	
$text .= "\n" . "Модель: " . "*" . $results[0]->name; 	
}
if ($year){
$text .= "\n" . "Год выпуска: " . "*" . $year; 	
}
